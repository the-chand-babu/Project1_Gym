...
const Register = () => {
    ...
    const Validation = (values) => {
        ...
        } else if (values.password.includes(123) || values.password.includes(1234) || values.password.includes(123456) || values.password.includes(12345678)) {
    ...
    return (
        <div>
            <h1>{
                isuserPresent ? "User Already exits please login" : ""
            }</h1>
            <h2>{isregister ? "Registration succesfully" : ""}</h2>
            <h1>Register</h1>
            <form className={styles.form} onSubmit={handelSubmit}>
                <label htmlFor="username"> UserName:</label>
                <input onChange={handelChange} value={formValues.username} type="text" id="username" name="username" />

                <p className={styles.errorMessage}>{error.username}</p>
                <label htmlFor="email">Email:</label>
                <input onChange={handelChange} value={formValues.email} type="text" id="email" name="email" />
                <p className={styles.errorMessage}>{error.email}</p>

                <label htmlFor="password">Password:</label>
                <input onChange={handelChange} value={formValues.password} type="password" id="password" name="password" />
                <p className={styles.errorMessage}>{error.password}</p>

                <label htmlFor="confirmPassword">Confirm Password:</label>
                <input onChange={handelChange} value={formValues.confirmPassword} type="password" id="confirmPassword" name="confirmPassword" />
